...
function palindromeThis(){
	...
	// var sorted_arr = total.toString().split('').slice().sort();
	var sorted_arr =  ["4", "6", "6", "7", "7", "7"]; //7767677
	var arr = [], noDup = [], mid = [];
	for(var i = 0, len = sorted_arr.length - 1;i < len;i++){
		if(sorted_arr[i] === (sorted_arr[i+1] || 0)){
			arr.push(sorted_arr[i]);
		}else{
			noDup.push(sorted_arr[i]);
		}
	}
	// noDup.forEach(function(val, i){
	...
	// var last = arr.join('');
	// var first = arr.sort(function(a, b){return b-a}).join('');
	// var midMax = Math.max.apply(null, mid);
	// var palindrome = ''.concat(first,midMax,last);
	// console.log('%c FLASH ', 'background: #800000; color: yellow; font-size: 14pt; font-family: "Comic Sans MS", cursive, sans-serif', arr);
	// return +palindrome;
}

palindromeThis(2824, 2399);
// palindromeThis(937,113);
